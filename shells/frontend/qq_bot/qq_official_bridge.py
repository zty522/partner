...
from shells.frontend.qq_bot.qq_official_bot import QQQfficialBot, QQMessage, QQMessageType, QQBotInfo
import queue as _queue
from partner.knowledge.knowledge import KnowledgeBase
from partner.journal import Journal, JournalEntry
from partner.state.state import StateManager
from partner.dialogue.outbound_policy import THINKING_NOTICE, UNAVAILABLE_NOTICE
from partner.state.config import (
    apply_runtime_agent_defaults,
    load_partner_config_data,
    save_partner_config_data,
)
from partner.core.interaction_orchestrator import InteractionOrchestrator
from partner.dialogue.user_text_safety import has_internal_diff, strip_internal_diff

logger = logging.getLogger(__name__)
from partner.tasks.task_queue import TaskQueue
...
class QQQfficialBridge:
    ...
    def _handle_message(self, msg: QQMessage):
        ...
        3. Enqueue message via event pipeline
        4. Poll for reply in qq_chat_history.jsonl
        """
        ...
        try:
            ...
            # No direct THINKING_NOTICE here — the mind executor handles progress notifications
            # via the unified delivery queue, ensuring ordered delivery and avoiding duplicates.
            ...
            if special_reply:  # Has a real reply
                self._send_reply_once(msg, special_reply, sub_key="special")
                return
            ...
            # Enqueue message via event pipeline (no direct orchestrator call)
    ...
    def _build_status_snapshot(self) -> Optional[Dict[str, str]]:
        ...
        try:
            from partner.projects.project_state import simplify_project_query
    ...
    def _summarize_project_state(self, focus_project: str) -> tuple[str, str, str]:
        if not focus_project:
            return "", "", ""
        try:
            from partner.projects.project_state import read_state_md
    ...
    def _summarize_recent_project_log(self, focus_project: str) -> str:
        try:
            from partner.projects.project_state import get_project_dir
    ...
    def _infer_focus_project(self) -> str:
        ...
        try:
            from partner.projects.project_state import get_active, resolve_project_name, simplify_project_query
        except Exception:
            return ""
    ...
    def _record_shared_content_signal(
        ...
        try:
            from partner.knowledge.content_feed import record_shared_content
    ...
    def _nudge_content_digest(self, item: dict):
        """保留兼容 — MindPool 已移除，Harness 接管调度。"""
        logger.debug("_nudge_content_digest no-op (MindPool removed, Harness active)")
    ...
    def _send_reply_once(self, original_msg: QQMessage, reply: str, sub_key: str = "reply"):
        """Send reply once per message+sub_key combo to deduplicate."""
        if not original_msg.msg_id:
            self._send_reply(original_msg, reply)
            return
        key = f"{original_msg.msg_id}:{sub_key}"
        now = time.time()
        cutoff = now - self._message_dedup_ttl
