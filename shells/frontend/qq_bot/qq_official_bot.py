...
class QQQfficialBot:
    ...
    async def upload_file(
        ...
        Args:
            ...
            file_name: Optional filename shown in QQ download UI.
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status in (200, 201):
                        ...
                        # QQ's upload response normally contains both
                        # file_uuid (upload receipt) and file_info (the token
                        # required by msg_type=7 media sending).  Sending
                        # file_uuid as media.file_info causes 40011000
                        # "请求数据异常".
                        file_info = result.get("file_info") or ""
                        file_uuid = result.get("file_uuid") or ""
                        if file_info:
                            logger.info(f"File uploaded: file_uuid={file_uuid or '<none>'}")
                            return file_info
                        if file_uuid:
                            logger.warning("Upload response missing file_info; file_uuid alone cannot be sent as media.file_info")
                            return None
                        else:
                        ...
                        logger.error(f"File upload failed: status={resp.status} body={text[:200]} url={endpoint} file_size={len(file_data)}")
                        return None
    ...
    async def _handle_c2c_message(self, data: Dict):
        ...
        msg = QQMessage(
            ...
            raw=data,
            extra={"attachments": self._extract_attachments(data)},
        )
    ...
    def _extract_attachments(self, data: Dict) -> List[Dict[str, Any]]:
        """Best-effort extraction of QQ attachment metadata from raw events."""
        rows: List[Dict[str, Any]] = []

        def add(value: Any, kind: str = ""):
            if not value:
                return
            ...
            if isinstance(value, dict):
                row = dict(value)
                row.setdefault("kind", kind or row.get("type") or row.get("file_type") or "attachment")
                url = (
                    row.get("url")
                    or row.get("file_url")
                ...
                name = row.get("filename") or row.get("file_name") or row.get("name") or row.get("id") or ""
                row["url"] = str(url or "")
                row["name"] = str(name or "")
                rows.append(row)
                return
            if isinstance(value, str):
                rows.append({"kind": kind or "attachment", "url": value if value.startswith("http") else "", "name": value})

        for key in ("attachments", "attachment", "media", "image", "images", "video", "file", "audio", "voice"):
