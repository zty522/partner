...
- Long-running work must use a persisted Campaign and bounded WorkItems. Process uptime, heartbeat,
  `Bot ready`, a written next step, or one passing task is not a continuous-run result.
...
- Long runs additionally produce CampaignState, WorkItem, InstanceLease, event history and CampaignReport records.
