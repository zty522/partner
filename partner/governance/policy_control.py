"""Conservative EGPL control plane: choose canary arms and decide them from v2 outcomes."""
...
from .experience_policy import effective_trajectory_reward, is_synthetic_trajectory
...
def assign_manual_canary(workspace: str, *, project_id: str, experiment_id: str,
    ...
    policy_dir = workspace_root(workspace) / "share" / "mind" / "governance" / "experience_guided_policy"
    prior = [row for row in _read_jsonl(policy_dir / "canary_assignments.jsonl")
    ...
    append_jsonl(policy_dir / "canary_assignments.jsonl", row)
...
def record_regression_attestation(workspace: str, *, experiment_id: str, command: str,
    ...
    path = (workspace_root(workspace) / "share" / "mind" / "governance" / "experience_guided_policy"
...
def _experiment_for(workspace: str, project_id: str, decision_key: str,
    ...
    issue = record_issue(workspace, {
        "summary": f"policy canary action selection: {decision_key}",
...
def choose_action(workspace: str, project_id: str, decision_key: str,
    ...
    experiment_id = ""
    if not selected:
        assignments = [row for row in _read_jsonl(policy_dir / "canary_assignments.jsonl")
    ...
    append_jsonl(policy_dir / "canary_assignments.jsonl", {
...
def evaluate_canaries(workspace: str, *, dry_run: bool = False) -> dict[str, Any]:
    """Write one promotion/rejection/inconclusive decision once both arms are testable.

    Args:
        workspace: Partner workspace root.
        dry_run: if True, do NOT write to control_policy.json.  Useful for
                 BDK ocamms hook validation — caller can see what would
                 happen without actually promoting.  Decisions are still
                 written to promotion_decisions.jsonl.
    """
    root = workspace_root(workspace)
    policy_dir = root / "share" / "mind" / "governance" / "experience_guided_policy"
    ...
    trajectory_rows = _read_jsonl(policy_dir / "trajectories.jsonl")
    latest_trajectories = {
        str(row.get("trajectory_id") or f"anonymous_{index}"): row
        for index, row in enumerate(trajectory_rows)
    }
    trajectories = [row for row in latest_trajectories.values()
                    if int(row.get("schema_version") or 0) >= 2
                    and not is_synthetic_trajectory(row)
    ...
    assignments = _read_jsonl(policy_dir / "canary_assignments.jsonl")
    ...
    result: dict[str, Any] = {"ok": True, "decisions": decisions}
    control_path = policy_dir / "control_policy.json"
    try:
        control = json.loads(control_path.read_text(encoding="utf-8"))
    except (OSError, ValueError):
    ...
    for key in sorted({str(row.get("decision_key")) for row in assignments if row.get("decision_key")}):
        ...
        attestation_path = policy_dir / "regression_attestations" / f"{experiment_id}.json"
        try:
        ...
        # ---- BDK ocamms promotion guard (opt-in via env) -----------------
        # When PARTNER_BDK_OCAMMS_ENFORCE=1 is set in the environment and the
        # candidate_id maps to a registered BDK-related skill, automatically
        # pass enforce_bdk_ocamms=True to decide_experiment.  This lets the
        # 05 promotion flow use BDK ocamms as an extra hard gate without
        # requiring callers to opt in per-call.
        enforce_bdk = False
        if candidate_id and os.environ.get("PARTNER_BDK_OCAMMS_ENFORCE") == "1":
            try:
                from partner.learn.bdk_ocamms_promotion_guard import (
                    _load_candidate,
                )
                cand = _load_candidate(workspace, candidate_id)
                if cand is not None:
                    intervention = cand.get("intervention") or {}
                    if isinstance(intervention, str):
                        try:
                            intervention = json.loads(intervention)
                        except (TypeError, ValueError):
                            intervention = {}
                    bdk_module = str((intervention or {}).get("bdk_module") or "")
                    if "bdk" in bdk_module.lower():
                        enforce_bdk = True
            except Exception as exc:  # noqa: BLE001 — best-effort detection
                # Fail-open: log but do not block promotion on guard detection error.
                _bdk_detection_log = str(exc)[:300]
        # -------------------------------------------------------------------

        decide_kwargs = {
            "experiment_id": experiment_id, "decision": decision,
            "evidence": [str(policy_dir / "trajectories.jsonl"), str(attestation_path),
        ...
        if enforce_bdk:
            decide_kwargs["enforce_bdk_ocamms"] = True
            decide_kwargs["candidate_id"] = candidate_id
        outcome = decide_experiment(workspace, decide_kwargs)
        if outcome.get("ok") and decision == "promoted":
            if not dry_run:
                control.setdefault("promoted", {})[key] = candidate_id
                control["updated_at"] = now_iso()
            else:
                # Dry-run: just record what *would* have been promoted.
                # Caller can read result["dry_run_promotions"] to audit.
                result.setdefault("dry_run_promotions", []).append({
                    "decision_key": key, "candidate_id": candidate_id,
                })
        decisions.append({"decision_key": key, "decision": decision, "metrics": metrics, "result": outcome})
    if not dry_run:
        atomic_json(control_path, control)
    result["control_policy_path"] = str(control_path)
