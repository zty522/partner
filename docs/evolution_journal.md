...
### 2026-09-01 — 从定时检查进化为任务终态驱动

- “每 5 分钟看一次”无法解决模型与双槽空闲；真正的时间单位应是权威 Task 终态。持久事实与低延迟唤醒
  分层：JSONL 用于恢复和审计，named FIFO 只负责马上叫醒 controller。
- 第一次实跑暴露 Candidate 外层 Event 名没有进入本地有界白名单。关注内部 handler 而忽略实际调度名，
  会让正确实现仍卡在错误执行通道；今后必须按 plan 中真实 `event_type` 验收。
- MiniMax DNS 失败证明“模型配置正确”不等于“实例启动环境可访问模型”。运行前网络能力是实验前置事实，
  environment failure 要与模型质量负样本分开。
- 产物存在也不是任务终态：Claim/Receipt 可以把初步 done 改判失败。完成信号只能来自最终治理，否则高效
  调度会放大假成功。相反，最终治理成功也不能因缺 generic LLM-check 而永远占槽。
- 高利用率不能靠删失败实现。baseline 缺证据、Candidate 单 Claim 语义失败和网络错误都保留；真正进步是
  下一轮选择受这些机制级负样本约束，同时 production readiness 继续 fail closed。
- “已经附加真证据”仍不等于产物整体通过：模型可能先写一套缺字段账本，机器再附一套正确账本。审计器
  正确地把两者一起判失败。可靠做法是明确所有权——LLM 负责正文，Candidate 运行时负责唯一账本；标题
  解析必须覆盖真实模型会写出的层级、中文序号和后缀，而不能只匹配理想模板。
- 修复后的在线 Candidate 随后得到真值通过与 Reward `+0.8`，但渠道阶段 ACK 仍缺失，所以业务送达分没有
  被一起抬高。这是当前闭环应保持的分离：内容质量改进可以成为学习正样本，未证实的用户可见交付不能。

### 2026-09-01 — 第一次真正由负结果推动的跨领域策略修正

- 主动学习不是自进化的同义词：前者选择最值得获取的下一个样本/问题，后者将失败诊断、最小 Candidate、
  matched 验证和受限策略变更闭合。Sprint 18 把两者串起，但仍分别落账。
- TargetDiff v2 把“不确定性+多样性+高 RMSD 切片”设为直觉 Candidate，真实数据证明它不如 MaxVar。
  系统没有删失败，而是把干预缩小：v3 降低多样性权重仍不足，v4 只保留 2% 多样性后反超 MaxVar。
  这是“观测→拒绝→缩小修复→再跑→指标改善”的真实初步证据，但单数据集不等于长期成熟。
- 在线调用 MiniMax 还暴露了“代码中有 Event ≠ manual preflight 允许调用”和“旧 canary 可能污染新匹配路径”。
  两者都必须修接线，不能用单元测试或报告绕过真实 Harness。

### 2026-09-01 — 单 seed 的“成功”被三 seed 主动证伪

- 主动学习的价值不只是找到更好样本，也包括主动寻找最可能推翻当前信念的证据。v4 在首个池胜出后，
  系统没有直接晋升，而是冻结模型、预算、official split 与 acquisition 权重，仅改变训练池随机种子。
- 两个新池都未复现优势：最终形成 1/3 胜出。均值略优并不能绕过预注册的复现门，因此策略被拒绝。
  这比反复运行同一 seed 获得更多“成功次数”更接近有意义的自进化。
- 同轮还发现了元学习层自己的观测盲区：真实证据落在 `active_learning` 时 Reward 看不见。修复后不覆盖
  旧负奖，而是追加可归因 revision。这使“系统学习如何学习”也遵守 Event-first 和历史不可改写。
- 当前能力是：能选择问题、保留反证、生成受限 Candidate、运行部分真实 matched、更新后验并拒绝脆弱策略。
  尚不能诚实声称：跨三真实日期、持续业务改善、自动 R2 代码修复普适成功或生产 RL 成熟。
- 更进一步，第一次 rejection 已自动触发一个可执行 R0 校准 Candidate，而非由人手选下一组权重。它只用
  labelled OOB residual 构造四个 RMSD slice 的 shrinkage factor，明确记录未访问 hidden pool label；但
  三 seed 仍 0/3。系统据此把下一假设缩到“先量化 uncertainty reliability curve”，并因无新证据回到等待。

### 2026-09-01 — 先验证“不确定性是否知道自己错了”，再谈主动采样

- 连续换 acquisition 权重没有回答核心问题：模型给出的 uncertainty 是否与真实误差相关。三 seed 诊断显示
  raw tree variance 与绝对误差近乎负相关，说明“有一个 uncertainty 数字”不等于它适合指导主动学习。
- 负结果应改变问题层级。课程没有再生成第三组拍脑袋权重，而是选择 estimator-comparison Recipe；只从已标注
  样本做 cross-fitting，避免 hidden test label 泄漏。Candidate 虽改善排序和 top-error recall，但相关性仍接近
  零，因此只获得 shadow 资格。这种克制是闭环的一部分，不是没做完。
- 学习账本和项目账本必须正交：learning artifact 可以证明一次诊断完成，却不能自动证明用户收到消息、项目有
  进展或生产策略有效。Campaign monitor 也只是控制面观察，不能被历史业务 policy 打分。
- namespace 既是存储结构也是证据边界。共享学习产物只能从 allowlisted Event 的输出字段投影；输入 source
  path、任意存在的文件或模型声称的路径都不能充当完成证据。
- 自动路由必须依赖 typed CandidateBundle 中的 recipe/mechanism，而不是报告文件名。文件名是展示细节，
  schema 才是可迁移合同。
- “持续运行”应理解为新证据到达后立即继续，而不是信息耗尽时循环旧任务。当前控制器在 29 项全终态后进入
  `WAITING_EVIDENCE` 并停止，保留三真实日期、20/arm、anchor 和 rollback 作为后续长期验证门。

### 2026-09-01 — 让控制器常驻，不等于让伪样本常驻

- 真正可持续的运行形态是进程可恢复、终态即时唤醒、空闲零模型调用；因此采用 Sprint18 专用 systemd 服务，
  而不是用通用 runner 假装具备主动课程续项能力。
- 第一小时报告暴露出控制面也会污染学习面：一个本地报告已记录、渠道 ACK 不完整的事件，同时产生 done/failed
  信号，进而耗掉 business failure budget。修复后渠道问题仍可见，但只进入 report ledger。
- 更重要的校正来自权威资格字段：旧 Observation 转换器把 75 条明确标为不可学习的轨迹重新算成可学习样本。
  样本数从 611 降到 539 不是退步，而是移除了训练污染。持续 RL 首先必须保证“什么不该学”也能被执行。
### 2026-09-02 — 持续运行的价值在跨日期新证据，不在重复旧题

- 常驻进程经历挂载 I/O 故障和 WSL 重启后能自动恢复，说明运行连续性已从会话内循环变成宿主级服务能力。
- 第二真实日期由日期事件自动生成 4 个 matched pair，Candidate 4/4 而 baseline 0/4；这是跨日改善证据，
  但旧的 5 个 Candidate false-success 不能因为今天表现好就删除。
- append-only 历史要求评估器区分“修订”和“新样本”。同一 trajectory 的 correction 若被重复计数，RL 的
  样本量、成功率和 Reward 都会失真；因此成熟度读取必须投影最新 revision，原始账本仍完整保留。
- 当前 Partner 已能一直在线并在新日期自动继续，但只有两个真实日期、Wilson 0.6545，仍应诚实保持 blocked。
### 2026-09-02 — 学习应由经历触发，不应由时钟触发

- 人在做项目时并不是每小时启动一次“学习任务”；失败、惊讶、矛盾、知识缺口和反馈才会触发反思与学习。
- Campaign 把资源调度、实验治理和实例认知混为一体，导致 02 在无新进展时发送阶段报告。控制器的存活不应
  冒充实例的行动。
- 合理分层是：实例拥有行动—观察—学习—再行动的局部闭环；资源仲裁器只守住最多双槽；Campaign 只封装
  需要严格预算和 baseline/candidate 的实验。定时器可以救活进程，但不能决定“现在该学什么”。
## 2026-09-02 — 从周期课程转向项目内元认知中断

- 观察：周期 Campaign 报告造成“看似一直运行、实际项目没推进”；重启后槽账本也可能与服务真值漂移。
- 假设：把 continuation owner 下沉到实例，以权威 Task terminal 驱动项目/学习切换，可同时保留手动消息体验和长期运行。
- 干预：实现实例原生状态机、双槽收敛、Episode 前置、确定性四 Event 学习、重启孤儿终结和 fail-fast planner。
- 真实结果：04 的 Episode 学习两次通过并立即恢复项目；过程中暴露 citations 误杀、无 Episode、handler fallback、
  artifact filename writer 等问题，均通过代码与回归修复，而非绕过。
- 决策：实例原生运行时进入生产；旧 Campaign 仅留实验用途。长期 RL 成熟度仍保持未通过，继续由真实业务结果验证。

## 2026-09-03 — 学习闭环也必须服从资源公平性

- 观察：04 的确定性学习可以成功，但应用到业务后若仍失败，旧 reducer 会清零连续失败并再次学习，累计八次中断，
  形成“学习很忙、项目不前进、其他实例轮不到”的新型假持续。
- 干预：把一次学习和紧随其后的业务重试视为同一个有界认知单元；重试仍失败则保留 Episode/Receipt 并让槽，
  后续轮转再访。控制器重启也只恢复既有非让出槽，不再按固定配置头部重选。
- 真实结果：04 和被旧重启打断的 01 均诚实让槽，05→02→03 随终态信号进入运行；02/03 随后各自完成一次
  Event-first 学习并恢复业务项目。全量 746 tests 通过。
- 认识：自进化次数不是价值指标；项目应用、真实终态、资源公平和其他项目获得行动机会共同构成长期闭环真值。
