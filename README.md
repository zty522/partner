...
|---------|---------------|
| **Auto-Install Agents** | When a specialized agent (e.g. xxx) is needed but not installed, Partner auto-downloads, installs, and configures it — no manual setup. |
...
Partner mind event (call_agent_skill)
    → external_agent_skills.py (two-tier dispatch)
        ...
        └── Specialized agents (xxx, bamboo-ai, ...)
...
```json
{
  ...
  "endpoint_config": {
    ...
    "args": [
      "-o", "{output}", "-q", "{question}",
      "-d", "{device}",
      "--analyses", "custom_task", "drivers_genes",
      "--report-format", "md",
      "{input}"
    ]
  },
...
```text
step1: atomic_ensure_agent_installed(agent="xxx")
       ...
       → register manifest
step2: call_agent_skill(agent="xxx", task="...")
...
Partner v1.0.0 supports **xxx Agent** (— — a specialized agent for data analysis, cell dynamics analysis, and differentiation studies.

- **No wrapper needed**: Partner calls `xxx-agent` directly — the real CLI.
- **LLM credentials auto-injected**: `~/.cellcompass/config.json` is written with Partner's API key at install time.
- **Dispatch**: `call_agent_skill(agent="xxx", ...)` → `xxx-agent --llm-* run -o OUTPUT -q QUESTION -d DEVICE --analyses custom_task drivers_genes INPUT.h5ad`
- **Auto-install**: First use triggers `atomic_ensure_agent_installed` which runs `pip install xxx-agent`.
- **CLI argument verification**: After install, Partner runs `xxx-agent run --help` and validates all manifest flags match.
...
| `xxx` | Data analysis (auto-installed on first use). |
...
**Important**: The xxx agent is an external dependency. Partner's repo only contains a manifest JSON describing how to call it. The actual xxx source code is maintained at its own repository and installed via `pip install xxx-agent`. The MIT license attribution in NOTICE.md covers the manifest integration layer only.
